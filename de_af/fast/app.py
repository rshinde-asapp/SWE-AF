"""de_af.fast.app — FastBuild Agent entry point.

Exposes:
  - ``app``: Agent instance with node_id='swe-fast'
  - ``build``: end-to-end fast build reasoner
  - ``main``: entry point for ``python -m de_af.fast`` and the ``swe-fast`` console script
"""
...
from de_af.reasoners import router as _execution_router  # noqa: E402
